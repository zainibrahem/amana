...
export default function SearchBar (props) {
    ...
    return (
      <div>
        <div className={data&&data.products.length >0|| Filter?"transition-all duration-300 overflow-hidden	 ease-in-out delay-300 rounded w-full flex flex-col justify-start items-center content-between  h-36 sm:h-80 bg-white shadow hover:shadow-md mt-7 mb-5 sm:mb-7":"transition-all overflow-hidden   delay-300 ease-in-out rounded w-full flex flex-col justify-start items-center content-between h-16 bg-white shadow hover:shadow-md mt-7 mb-5 sm:mb-7 duration-300"}>
        <div className="w-full flex justify-around items-center pt-3">
            <div className='w-11/12 relative'>
                <input type="text" dir="rtl" name="searchs" onChange={toggleSearch} placeholder="البحث عن المنتجات في أمانة..." className={`rounded  w-full pr-2 border-2 focus:outline-none text-right text-sm py-1 ${Loading?"skeleton-box":""}`}/>
                <button style={Loading?{opacity:"0"}:{}} className={`text-xs sm:text-md  focus:outline-none flex justify-between items-center absolute left-0 top-0 h-full px-3 sm:px-5 lg:px-10 rounded-r-none rounded bg-yellow-500`}>
        ...
        <div className={data&&data.products.length>0&&!Filter?"mt-6 sm:mt-8 w-full":"mt-6 hidden sm:mt-8 w-full"}>
            <div className="grid grid-cols-12 gap-4">
                    ...
                    <ul className="px-2">
                        {data?data.products.map((ele,index)=>
                            ...
                            <li className="flex justify-end items-center">
                                ...
                                <span dir="rtl" className="w-7/12 breaks whitespace-nowrap text-xs font-bold text-right text-gray-500 ml-1  overflow-hidden"> {ele.title} 
                                </span>
                            </li>    
                            :""
                        ):""}
                ...
                <div className="col-span-2 hidden xl:block hover:bg-gray-100 p-2 rounded">
                    {data&&data.special_item?
                    <Link href={`/product/product?pids=${data.special_item.id}`}>
                        <div className="border-green-500 border-r-2 flex flex-col cursor-pointer justify-around items-end px-3   h-full choosen">
